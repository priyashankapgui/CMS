...
export const WebFeedbacks = () => {
    ...
    const [rows, setRows] = useState(jsonData.feedbackRowData);

    // Sort rows based on receivedAt date property in descending order
    const sortedRows = rows.slice().sort((a, b) => new Date(b.receivedAt) - new Date(a.receivedAt));
                                ...
                                {currentRows.map((row, index) => (
                                    <React.Fragment key={index}>
                                        ...
                                        {openRowIndex === index && (
                                            <tr>
                                                <td colSpan="7">
                                                    <div className="details" style={{ backgroundColor: row.action === 'Taken' ? 'rgba(76, 215, 101, 0.6)' : 'rgba(255, 0, 0, 0.5)' }}>
