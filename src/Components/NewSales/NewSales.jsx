...
import radioBtnOptions from '../Data.json';
...
function NewSales() {
    ...
    const fetchProductSuggestions = async (searchTerm) => {
        try {
            ...
            const response = await axios.get(`http://localhost:8080/product-batch-sum/branch/${selectedBranch}`);
            const data = response.data;
            return data
                .filter(item => {
    ...
    const handleProductSuggestionSelect = (suggestion, id) => {
        setRows(rows.map(row =>
            row.id === id
                ? {
                    ...row,
                                ...
                                <tr key={row.id}>
                                    ...
                                    <td>
                                        <SearchBar
                                            ...
                                            onSelectSuggestion={(suggestion) => handleProductSuggestionSelect(suggestion, row.id)}
